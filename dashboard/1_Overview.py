...
st.markdown(
    ...
    "Data: Insecurity Insight / SHCC  ·  Oct 2023–Feb 2026  ·  "
    "Code: github.com/toddlerjacker/pse-healthcare-attacks<br>"
...
fig_timeline = px.bar(
    monthly,
    x="Month_Date", y="Incidents", color="Killed",
    color_continuous_scale=[
        [0.0, "#d0d0d0"], [0.1, "#f5c97a"], [0.4, "#e8916a"], [1.0, "#c0392b"],
    ],
...
fig_timeline.add_vline(
    ...
    line_dash="dash", line_color="rgba(0,0,0,0.3)",
    annotation_text="Oct 7 2023", annotation_position="top right"
)
...
kv_monthly = (
    filtered.groupby("Year_Month")
    .agg(Killed=("Health Workers Killed", "sum"), Injured=("Health Workers Injured", "sum"))
    .reset_index()
    .sort_values("Year_Month")
)
kv_monthly["Month_Date"] = pd.to_datetime(kv_monthly["Year_Month"] + "-01")
kv_melt = kv_monthly.melt(
    id_vars="Month_Date", value_vars=["Killed", "Injured"],
    var_name="Type", value_name="Count"
)

fig_kv = px.area(
    kv_melt, x="Month_Date", y="Count", color="Type",
    color_discrete_map={"Killed": "#c0392b", "Injured": "#e8916a"},
    labels={"Month_Date": "Month", "Count": "Health Workers", "Type": ""},
    line_group="Type",
)
fig_kv.update_traces(hovertemplate="<b>%{x}</b><br>%{fullData.name}: %{y}<extra></extra>")
fig_kv.add_vline(
    x=pd.Timestamp("2023-10-07").timestamp() * 1000,
    line_dash="dash", line_color="rgba(0,0,0,0.4)",
    annotation_text="Oct 7 2023", annotation_position="top right"
)
...
with col1:
    ...
    w_metric = st.radio("Show by", ["Incidents", "Killed", "Injured"],
                        horizontal=True, key="weapon_metric")
    weapon_agg = (
        weapon_df_full.groupby("Weapon_Category")
        .agg(Incidents=("Date", "count"),
             Killed=("Health Workers Killed", "sum"),
             Injured=("Health Workers Injured", "sum"))
        .reset_index().sort_values(w_metric, ascending=True)
    )
    ...
    fig_weapon = px.bar(
        weapon_agg, x=w_metric, y="Weapon_Category", orientation="h", color=w_metric,
        color_continuous_scale=[[0.0,"#e8e8e8"],[0.5,"#e8916a"],[1.0,"#c0392b"]],
        labels={"Weapon_Category": "", "Killed": "HW Killed", "Injured": "HW Injured"},
        text="Label"
    )
    fig_weapon.update_traces(textposition="outside", cliponaxis=False,
...
with col2:
    ...
    p_metric = st.radio("Show by", ["Incidents", "Killed", "Injured", "Arrested"],
    ...
    perp_agg = (
        ...
        .reset_index().sort_values(p_metric, ascending=True)
    )
    ...
    fig_perp = px.bar(
        perp_agg, x=p_metric, y="Perpetrator_Simple", orientation="h", color=p_metric,
        color_continuous_scale=[[0.0,"#e8e8e8"],[0.5,"#e8916a"],[1.0,"#c0392b"]],
        labels={"Perpetrator_Simple": "", "Killed": "HW Killed",
                "Injured": "HW Injured", "Arrested": "HW Arrested"},
        text="Label"
    )
    fig_perp.update_traces(textposition="outside", cliponaxis=False,
...
phase_melt = phase_df.melt(
    id_vars="Conflict_Phase", value_vars=["Killed", "Injured", "Arrested"],
    var_name="Casualty Type", value_name="Count"
)
fig_phase = px.bar(
    phase_melt, x="Conflict_Phase", y="Count", color="Casualty Type", barmode="group",
    color_discrete_map={"Killed": "#c0392b", "Injured": "#e8916a", "Arrested": "#7fb3d3"},
    labels={"Conflict_Phase": "", "Count": "Health Workers", "Casualty Type": ""},
)
fig_phase.update_traces(hovertemplate="<b>%{x}</b><br>%{fullData.name}: %{y}<extra></extra>")
...
target_data = {
    "Type": ["Health Workers Killed", "Health Workers Injured", "Health Workers Arrested",
             "Facility Attacked", "Transport Attacked", "Access Disrupted"],
    "Count": [
    ...
    "Category": ["Personnel", "Personnel", "Personnel",
                 "Infrastructure", "Infrastructure", "Infrastructure"]
}
target_df = pd.DataFrame(target_data).sort_values("Count", ascending=True)
fig_target = px.bar(
    target_df, x="Count", y="Type", orientation="h", color="Category",
    color_discrete_map={"Personnel": "#c0392b", "Infrastructure": "#7fb3d3"},
    labels={"Type": "", "Count": "Count", "Category": ""}, text="Count"
)
...
# ── Weapon use by conflict phase (the one heatmap) ────────────────────────────
...
pivot = heat_df.pivot_table(
    index="Weapon_Category", columns="Conflict_Phase",
    values="Incidents", fill_value=0
)
...
fig_heat = px.imshow(
    ...
    color_continuous_scale=[
        [0.0, "#1a1a2e"], [0.15, "#2d1b4e"], [0.4, "#8b2635"],
        [0.7, "#c0392b"], [1.0, "#ff6b6b"],
    ],
...
stacked_df = stacked_df[(stacked_df["Killed"] > 0) | (stacked_df["Injured"] > 0)]
...
stacked_melt = stacked_df.melt(
    ...
    value_vars=["Pct_Killed", "Pct_Injured"], var_name="Type", value_name="Percentage"
)
stacked_melt["Type"] = stacked_melt["Type"].map({"Pct_Killed": "Killed", "Pct_Injured": "Injured"})
stacked_melt["Raw_Count"] = stacked_melt.apply(
    lambda r: r["Killed"] if r["Type"] == "Killed" else r["Injured"], axis=1
)

fig_stacked = px.bar(
    stacked_melt, x="Percentage", y="Weapon_Category", color="Type",
    orientation="h", barmode="stack",
    color_discrete_map={"Killed": "#c0392b", "Injured": "#e8916a"},
    labels={"Weapon_Category": "", "Percentage": "% of casualties", "Type": ""},
    text="Percentage", hover_data=["Raw_Count", "Total"]
)
for _, row in stacked_df.iterrows():
    fig_stacked.add_annotation(
        x=100.5, y=row["Weapon_Category"], text=f"n={row['Total']}",
        showarrow=False, font=dict(size=10, color="#888888"), xanchor="left"
    )
fig_stacked.update_traces(
    texttemplate="%{text:.1f}%", textposition="inside",
    hovertemplate="<b>%{y}</b><br>%{fullData.name}: %{x:.1f}%<br>Count: %{customdata[0]}<extra></extra>"
)
fig_stacked.update_layout(
    plot_bgcolor="#f7f5f2", paper_bgcolor="#f7f5f2", font_color="#333333",
    xaxis=dict(gridcolor="#eeeeee", ticksuffix="%", range=[0, 115]),
    legend=dict(orientation="h", yanchor="bottom", y=1.02),
    margin=dict(t=30, b=10, r=80), height=350
)
